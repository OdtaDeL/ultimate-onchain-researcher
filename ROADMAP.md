...
- [x] **Moved the daily automated pipeline off Vercel Cron to GitHub Actions, and off ChainBroker entirely** — two separate, unrelated production blockers found and fixed the same day:
  ...
  2. **ChainBroker cannot be called from GitHub Actions at all** — see the Hot Projects entry above for the confirmation. Rather than let a guaranteed-to-fail step block everything downstream, `.github/workflows/sync.yml` now runs **only** market metrics (CoinGecko → DefiLlama → CoinPaprika → DexScreener, all confirmed working from GitHub Actions) + scoring. ChainBroker sync (`npm run sync:all` — projects/funds/funding-rounds/unlocks) is manual-only now, run locally by whoever has residential-IP access to `api.chainbroker.io`. This was an explicit product decision (not just a technical workaround): the existing project catalog is accepted as the baseline, and automated effort is spent maximizing metrics completeness on those projects rather than trying to keep the catalog itself continuously fresh.
