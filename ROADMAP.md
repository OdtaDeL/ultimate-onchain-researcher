...
- [ ] **IN PROGRESS, paused on an external block — ChainBroker per-project metrics/TVL + Hot Projects pipeline.** Two pieces, both requested 2026-07-06:
  ...
  - **Blocked, and the block is now understood precisely (updated 2026-07-06)**: chainbroker.io timed out from this environment partway through the original session — initially assumed a temporary Cloudflare soft-block from heavy crawl volume. **Now confirmed to be an IP-range block, not a transient outage**: a real GitHub Actions run hit `ProviderRetriesExhaustedError` on all 4 ChainBroker endpoints (full retry exhaustion, every attempt), while the identical requests succeeded immediately from a residential IP. Retrying from a CI/datacenter environment will not help; this only works from a residential-IP machine today.
  - **Not started**: the raw Zod schemas + `client.ts` normalizers for these 3 endpoints. Deliberately not written from guesswork — every other `Raw*` type in `chainbroker/types.ts` carries a "confirmed against GET ..." comment, and coding these blind risks a full rewrite once the real field names are seen. **Migration `012` has NOT been applied to production yet** — apply it only once the ingestion code that populates the new columns is ready, so it doesn't sit live-but-unused.
  - **To resume**: from a residential-IP machine (not CI), `curl https://api.chainbroker.io/api/v1/projects/trending/` to get a real sample, record it in `SOURCE.md` (same as every other endpoint there), implement the Raw* schemas + client.ts normalizers + provider interface methods, then decide how "Hot Projects" gets refreshed given ChainBroker can't be called from GitHub Actions — likely a manual/local-only job (`npm run sync:trending`, mirroring how ChainBroker's other jobs work now — see the automated-pipeline entry below), not part of `.github/workflows/sync.yml`.
- [x] **Moved the daily automated pipeline off Vercel Cron to GitHub Actions, and off ChainBroker entirely** — two separate, unrelated production blockers found and fixed the same day:
  1. **Vercel Cron never actually worked**: this project runs on Vercel's **Hobby (free) plan**, confirmed with the user — `maxDuration = 300` in `src/app/api/cron/sync/route.ts` only takes effect on Pro; Hobby hard-caps every serverless function at **10 seconds**, and the pipeline takes tens of minutes. The daily cron in `vercel.json` had almost certainly been silently timing out since it was first set up. Fixed by moving the schedule to `.github/workflows/sync.yml` (GitHub Actions, no such ceiling — up to 6h/job on the free tier) and removing `crons` from `vercel.json`. Required creating a GitHub repo for this project for the first time (`github.com/OdtaDeL/ultimate-onchain-researcher`, public) and adding `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` as repository secrets.
  2. **ChainBroker cannot be called from GitHub Actions at all** — see the Hot Projects entry above for the confirmation. Rather than let a guaranteed-to-fail step block everything downstream, `.github/workflows/sync.yml` now runs **only** market metrics + scoring. ChainBroker sync (`npm run sync:all` — projects/funds/funding-rounds/unlocks) is manual-only now, run locally by whoever has residential-IP access to `api.chainbroker.io`. This was an explicit product decision (not just a technical workaround): the existing project catalog is accepted as the baseline, and automated effort is spent maximizing metrics completeness on those projects rather than trying to keep the catalog itself continuously fresh.
- [x] **Found and fixed a real data-correctness bug: cross-provider ticker collisions were silently overwriting projects with the wrong coin's data.** Concrete example that surfaced it: Hyperlane (HYPER) showed `price=2.1e-8, market_cap=0` — off by ~3.6 million from its real ~$0.07/$12.8M. Root cause: **ticker squatting is the rule in crypto, not the exception** — confirmed live, DexScreener alone returns 24 different, unrelated tokens all sharing the exact symbol "HYPER" (Bitcoin Hyper, Hyperpigmentation, Hyper Ape AI, the real Hyperlane, ...). Identity resolution only checked whether a symbol was unique *in this platform's own project catalog*, never whether the *provider's own batch* contained multiple different coins sharing it — so whichever colliding record got processed last silently won, overwriting the project with an unrelated coin's price/market cap. This affected CoinGecko most (16,000+ coins → highest collision surface) but structurally could hit any symbol/name-matching provider.
  - **Fix**: `src/ingestion/metrics/ingestion-service.ts` now counts symbol/name occurrences within each provider's batch before resolving anything; a symbol/name shared by 2+ different records in that batch gets blanked on the draft's identity, so resolution falls through to a safer tier (or unresolved) instead of guessing — same "never fabricate a match" principle `src/identity/` already applied to cross-project ambiguity, now applied to cross-provider ambiguity too.
  - **Also flipped provider priority (explicit ask)**: CoinPaprika (~2,000 coins, smaller/safer catalog) and DexScreener (per-project exact-ticker search) now run FIRST as the primary market-data source; CoinGecko now only fills what they left null (`fillNullsOnly`) and runs last. Updated in `syncMetrics.ts`, `metrics-sync.ts`, `cron.ts`, and the GitHub Actions workflow.
  - **Also fixed**: a 1-character ticker ("U") caused DexScreener's search to 400 and abort the entire gap-fill job with no per-project isolation — added a try/catch per project plus a minimum query-length filter, mirroring the resilience pattern `src/sync/chainbroker/paged-sync.ts` already has.
  - **Applied to production**: wiped `project_metrics` (809 rows) and `project_aliases` (1,302 rows) — kept `projects` (names/slugs/tickers) untouched — then re-ran the full pipeline in the new order. Verified: Hyperlane now shows the correct ~$0.073/$12.8M. Coverage held steady (~34% price/market-cap, ~5% TVL — this fix targeted correctness, not coverage, by design). Deployed 2026-07-06.
- [x] **Found and fixed a second, much larger bug the same day: identity resolution was silently blind to ~1,251 of 2,251 projects.** Surfaced by a user report — Aave (a top-50 CoinGecko project, obviously correct upstream data on every provider) had zero `project_metrics` and zero `project_aliases` rows, for any provider. Root cause: `fetchProjectCandidates`/`fetchAliasesForProvider` in `src/identity/resolver.ts` both did a plain, unpaginated `.select()` — PostgREST caps every response at 1,000 rows server-side (same behavior already worked around once before in `scoring-sync.ts`'s `loadTargetProjects`, but never applied here). With no `ORDER BY`, which ~1,251 projects fell outside that window was effectively arbitrary — Aave just happened to be one of them, structurally unmatchable by every provider regardless of how correct that provider's data was.
  - **Fix**: paginated both functions via `.range()`, mirroring the existing fix.
  - **Applied to production**: wiped `project_metrics`/`project_aliases` again and re-ran the full pipeline. Impact was immediate and large — matched-project counts roughly doubled at every stage (CoinPaprika 323→628, DexScreener 158→161→ now against a cleaner gap set, DefiLlama 171→393, CoinGecko 1,244→2,329). **Final coverage: price/market-cap 34%→65.7% (1,478/2,251), TVL 5.2%→13.6% (307/2,251).**
  - **Aave's remaining gap (TVL) was a separate, real modeling mismatch, not a bug**: DefiLlama tracks Aave as multiple versioned protocols (`aave-v3` $12.66B TVL, `aave-v2`, `aave-v1`, `aave-horizon-rwa`, `aave-arc`, ...), none named/slugged exactly "Aave" — so it could never auto-match. Asked the user how to handle it (map to V3 only vs. sum all versions vs. leave unmatched); user chose **V3 only** (the dominant, currently-active deployment). Registered via `applyManualOverride`'s existing mechanism (`project_aliases`, confidence 100, `manual_override` tier) — the intended process for exactly this kind of human-reviewed exception. This same multi-version-protocol pattern likely recurs for other major DeFi protocols (Uniswap, Compound, etc.) — not addressed systematically, flagged as backlog. Deployed 2026-07-06/07.
- [ ] **Backlog**: other major multi-version protocols (Uniswap, Compound, etc.) likely have the same "DefiLlama splits by version, we track one project" gap Aave had — not yet audited or manually overridden.
