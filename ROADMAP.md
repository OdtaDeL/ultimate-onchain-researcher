...
| T-002 | No scheduled ingestion — data freshness is 100% manual | Reliability (stale rankings mislead users) | `src/cli/` | Add cron (Vercel Cron or Supabase Edge) |
...
| T-007 | `project_metrics` rows may be absent for newly ingested projects — Markets Projects tab honest-drops these rows, showing fewer items than the ranking MV contains | UX correctness | `src/lib/api/sources/markets.ts` | Ensure `sync:metrics` runs after `sync:projects` in the scheduled job |
